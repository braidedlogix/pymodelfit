...
class ParametricModel(object,metaclass= ABCMeta ):
    """
    ...
    * :meth:`_filterfunc`
    ...
    def resampleFit(self,x=None,y=None,xerr=None,yerr=None,bootstrap=False,
        ...
        if plothist:
            ...
            params = (plothist,) if isinstance(plothist,str) else self.params
...
class FunctionModel1DAuto(FunctionModel1D, metaclass=AutoParamsMeta):
    """
...
class DatacentricModel1DAuto(DatacentricModel1D, metaclass= AutoParamsMeta):
    """
        ...
        with mpl_context(clf=clf) as plt:
            ...
            for i,m in enumerate(self.models):
                if fixedlabel is None:
                    label = ['%s=%.2g'%t for t in m.pardict.items() if t[0] in labelpars]
                    for ep,pv in self._extraparams.items():
                        if ep in labelpars:
...
class FunctionModel2DScalar(FunctionModel,InputCoordinateTransformer):
    ...
    def plot(self,datarange=None,nx=100,ny=100,clf=True,cb=True,data='auto',
        ...
        with mpl_context(clf=clf) as plt:
            ...
            if log:
                if isinstance(log,str) and 'mag' in log:
...
class FunctionModel2DScalarAuto(FunctionModel2DScalar, metaclass = AutoParamsMeta):
    """
...
def list_models(include=None,exclude=None,baseclass=None,showabstract=False):
    ...
    res = [k for k,c in __model_registry.items()
    ...
    if include is not None:
        ...
        for m in include:
            if isinstance(m,str):
                ...
                for nm,cls in __model_registry.items():
                    if m is cls:
                        valid.append(nm)
                        if nm not in res:
